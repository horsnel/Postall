...
import {
  LayoutDashboard, ClipboardList, FileText, ShoppingCart, MessageCircle, Wallet, CircleUserRound,
  Settings, Bell, Heart, Star, Menu, LogIn, ChevronRight, Briefcase, Building2, Compass,
  SlidersHorizontal, Search, Zap, Users, Calendar, Eye, Sparkles, Bookmark, History,
} from 'lucide-react'

type SidebarItem = { label: string; href: string; icon: React.ComponentType<{ className?: string }>; badge?: number; roles?: UserRole[] }
...
function getVisibleItems(role?: UserRole): SidebarItem[] {
  ...
  if (role) {
    for (const item of roleSpecificItems) {
      if (item.roles?.includes(role) && !visible.some(v => v.href === item.href)) visible.push(item)
    }
  }
  return visible
}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-5">
        <div className="h-9 w-9 rounded-lg bg-primary flex items-center justify-center"><span className="text-primary-foreground font-bold text-sm">PA</span></div>
        <div><span className="font-bold text-lg">PostAll</span><p className="text-[11px] text-muted-foreground -mt-0.5">{user?.role ? user.role.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Dashboard'}</p></div>
      </div>
      <Separator />
          ...
          {items.map((item) => {
            ...
            return (
              <Link key={`${item.label}-${item.href}`} href={item.href} onClick={onItemClick}
                ...
                {item.badge && <Badge variant="secondary" className={`h-5 min-w-[20px] justify-center px-1.5 text-[10px] ${isActive ? 'bg-primary-foreground/20 text-primary-foreground' : 'bg-emerald-100 text-emerald-700'}`}>{item.badge}</Badge>}
              </Link>
            )
          })}
        </nav>
      ...
      <div className="p-4">
        {user && (
          <Link href="/dashboard/profile" onClick={onItemClick} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-accent transition-colors">
            ...
            <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{user.username}</p><p className="text-[11px] text-muted-foreground truncate">{user.role?.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) || ''}</p></div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4 max-w-md mx-auto p-6">
          <div className="h-16 w-16 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto"><LogIn className="h-8 w-8 text-primary" /></div>
